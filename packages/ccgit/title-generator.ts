...
function generateFallbackTitle(
  ...
  if (metadata.prompt) {
    ...
    // Common patterns
    if (shortPrompt.includes("fix")) {
      return "fix: " + summarizePrompt(metadata.prompt);
    }
    if (shortPrompt.includes("add")) {
      return "feat: " + summarizePrompt(metadata.prompt);
    }
    if (shortPrompt.includes("update")) {
      return "chore: " + summarizePrompt(metadata.prompt);
    }
    if (shortPrompt.includes("refactor")) {
      return "refactor: " + summarizePrompt(metadata.prompt);
    }
    if (shortPrompt.includes("test")) {
      return "test: " + summarizePrompt(metadata.prompt);
    }
    if (shortPrompt.includes("doc")) {
      return "docs: " + summarizePrompt(metadata.prompt);
    }
  }
  ...
  if (categories.docs.length > 0) {
    return "docs: update documentation via Claude";
  }
  if (categories.config.length > 0) {
    return "config: update configuration via Claude";
  }
  if (categories.build.length > 0) {
    return "build: update build files via Claude";
  }
  if (files.some((f) => f.status === "A")) {
    return "feat: add new files via Claude";
  }
  if (files.some((f) => f.status === "D")) {
    return "chore: remove files via Claude";
  }
